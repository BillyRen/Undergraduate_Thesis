...
\subsection{定位时间}
对于定位时间的估计，我采取的方法是：在每种环境下，分别处理500张图像，计算其定位时间。

详细定位时间统计见表（\ref{tab:time}）。测试用图像尺寸为320$\times$240
...
\chapter{总结}
\section{工作总结}
本文主要从图像处理的角度出发，研究了二维码识别相关领域技术，提出了具有高容错性的快速定位算法。在之前的研究中，定位320$\times$240图像的时间约为3ms。 本研究提供的算法定位时间在1ms左右，将定位效率提升了3倍，效率超过以往的研究。本研究今后可以在工业上进行测试使用，可以极大地提高流水线识别效率，提高生产效率，创造额外价值。这是本研究最大的两点和价值所在。
